...
title "2a PH Regression Analysis Using Efron";
proc phreg data=q2;
model year*status(0) = trta / ties=efron;
run;

title "2a PH Regression Analysis Using Exact";
proc phreg data=q2;
model year*status(0) = trta/ ties=exact;
run;

title "2a PH Regression Analysis Using Breslow";
proc phreg data=q2;
model year*status(0) = trta / ties=breslow;
run;
...
title "4a Univariate analysis of treatment effect using treatment 1";
proc phreg data=q4;
model month*event(0) = trt;
run;

title "4b Score test of treatment effect adjusting for other covariates";
proc phreg data=q4;
model month*event(0) = trt drinking smoking age gender
/ selection=stepwise include=5 details slentry=1.0;
run;

title "4c Model Selection";
proc phreg data=q4;
model month*event(0) = trt drinking smoking age gender
/ selection=forward include=1 details slentry=0.1 slstay=0.15;
run;

title "4d Correlation Analysis";
proc corr data=q4;
var trt ID event drinking smoking age gender month;
run;
...
title "5a Correlation Analysis";
proc corr data=q5;
var ID month event trt meno tsize nodes er;
run;

title "5b Univariate analysis of treatment effect using treatment 1";
proc phreg data=q5;
model month*event(0) = trt;
run;

title "5c Score test of treatment effect adjusting for other covariates";
proc phreg data=q5;
model month*event(0) = trt meno tsize nodes er
/ selection=stepwise include=5 details slentry=1.0;
run;

title "5d Model Selection";
proc phreg data=q5;
model month*event(0) = trt meno tsize nodes er
/ selection=stepwise include=1 details slentry=0.1 slstay=0.1;
run;

title "5e Univariate analysis of tumor size";
proc phreg data=q5;
model month*event(0) = tsize;
run;

title "5f Multivariate analysis of tumor size";
proc phreg data=q5;
model month*event(0) = tsize meno nodes er;
run;