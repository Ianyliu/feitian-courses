...
proc lifetest data=HW7 nelson plot=(s(CL) ls lls h) ;
time month*event(0);
strata trt;
run;
...
proc phreg data=HW7;
model month*event(0) = trt meno tsize nodes er/selection=stepwise include=1 details slentry=0.1 slstay=0.1;
run;
